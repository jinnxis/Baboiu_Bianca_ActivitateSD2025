...
struct Biblioteca* citesteDinFisier(const char* numeFisier, int* dim) {
	...
	*dim = 0;
	while (fgets(linie, sizeof(linie), f)) {
		vector = (struct Biblioteca*)realloc(vector, (*dim + 1) * sizeof(struct Biblioteca));
		...
		token = strtok(NULL, ";");
		int nrCarti = atoi(token);
		token = strtok(NULL, ";\n");
		float timp = (float)atof(token);
		vector[*dim] = initializare(id, denumire, nrCarti, timp);
		(*dim)++;
	}
	fclose(f);
	return vector;
...
void scrieObiectInFisier(const char* numeFisier, struct Biblioteca b) {
...
void scrieVectorInFisier(const char* numeFisier, struct Biblioteca* vector, int dim) {
	FILE* f;
	errno_t err = fopen(numeFisier, "w");
	if (err != 0 || f == NULL) {
		printf("Eroare la crearea fisierului %s!\n", numeFisier);
		return;
	}
	for (int i = 0; i < dim; i++) {
		fprintf(f, "%d;%s;%d;%.2f\n", vector[i].id, vector[i].denumire, vector[i].nrCarti, vector[i].timp);
	}
	fclose(f);
	printf("Fișierul %s a fost creat și completat cu succes.\n", numeFisier);
}
...
int main() {
	...
	int dim = 0;
	struct Biblioteca* vector = citesteDinFisier("biblioteci.txt", &dim);

	printf("Biblioteci citite din fisier: \n");
	afisareVector(vector, dim);

	struct Biblioteca bNou = initializare(11, "Biblioteca Noua", 1000, 1.0);
	scrieObiectInFisier("biblioteci.txt", bNou);

	scrieVectorInFisier("biblioteci2.txt", vector, dim);
